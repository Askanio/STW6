...
namespace ScrewTurn.Wiki.Web.Controllers
{
    public class HomeController : PageController
    {
        ...
        {
            var model = GeneratePage(id);
            return View("WikiPage", model);
        }

        [HttpGet]
        public ActionResult PageDiscuss(string id)
        {
            DiscussMode = true;
            var model = GeneratePage(id);
            return View("WikiPage", model);
        ...
        [HttpGet]
        public ActionResult PageViewCode(string id)
        {
            ViewCodeMode = true;
            var model = GeneratePage(id);
            return View("WikiPage", model);
        ...
        private WikiPageModel GeneratePage(string pageName)
        {
        ...
        private string DetectCurrentNamespace(string pageName)
        {
        ...
        private void VerifyAndPerformRedirects()
        {
            if (CurrentPage == null)
            {
                UrlTools.Redirect(UrlTools.BuildUrl(CurrentWiki, "PageNotFound.aspx?Page=", Tools.UrlEncode(Tools.DetectCurrentFullName()))); // TODO:
            }
            if (Request["Edit"] == "1")
            {
