...
namespace ScrewTurn.Wiki.Plugins.AzureStorage {
	...
	/// </summary>
	public class AzureUsersStorageProvider : IUsersStorageProviderV60 {
		...
		#region IUsersStorageProviderV50 Members
		...
		private UserEntity GetUserEntity(string wiki, string username) {
			...
			if(!_users.ContainsKey(username)) {
                CloudTable table = _cloudTableClient.GetTableReference(UsersTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, username);
                ...
                var entity = table.ExecuteQuery<UserEntity>(query).FirstOrDefault();
		...
		private IList<UserEntity> GetUsersEntities(string wiki) {
			if(!(allUsersRetrieved && _users != null)) {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(UsersTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                TableQuery<UserEntity> query = (new TableQuery<UserEntity>()).Where(partitionKeyFilter);
                var entities = table.ExecuteQuery<UserEntity>(query).ToList();
		...
		public UserInfo ModifyUser(UserInfo user, string newDisplayName, string newPassword, string newEmail, bool newActive) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(UsersTable);
                TableOperation operation = TableOperation.Replace(oldUserEntity);
                table.Execute(operation);
		...
		public bool RemoveUser(UserInfo user) {
			...
			try {
				...
				foreach(UserDataEntity userDataEntity in userDataEntities) {
                    batchOperation.Delete(userDataEntity);
				}
                if (batchOperation.Count > 0)
                    table.ExecuteBatch(batchOperation);

                TableOperation operation = TableOperation.Delete(userEntity);
                table.Execute(operation);
		...
		private UserGroupEntity GetUserGroupEntity(string wiki, string groupName) {
			...
			if(!_userGroups.ContainsKey(groupName)) {
                ...
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, groupName);
                string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
                TableQuery<UserGroupEntity> query = (new TableQuery<UserGroupEntity>()).Where(combinedFilter);
                var entity = table.ExecuteQuery<UserGroupEntity>(query).FirstOrDefault();
		...
		private IList<UserGroupEntity> GetUserGroupsEntities(string wiki) {
			if(!(allGroupsRetrieved && _userGroups != null)) {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(UserGroupsTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                TableQuery<UserGroupEntity> query = (new TableQuery<UserGroupEntity>()).Where(partitionKeyFilter);
                var entities = table.ExecuteQuery<UserGroupEntity>(query).ToList();
		...
		public UserInfo SetUserMembership(UserInfo user, string[] groups) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(UsersTable);
                TableOperation operation = TableOperation.Replace(userEntity);
                table.Execute(operation);
		...
		public UserInfo GetUserByEmail(string email) {
			...
			try {
                ...
                string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, emailFilter);
                TableQuery<UserEntity> query = (new TableQuery<UserEntity>()).Where(combinedFilter);
                UserEntity userEntity = table.ExecuteQuery<UserEntity>(query).FirstOrDefault();
                //var userQuery = (from e in _context.CreateQuery<UserEntity>(UsersTable).AsTableServiceQuery()
		...
		private UserDataEntity GetUserDataEntity(string _wiki, string username, string key) {
            CloudTable table = _cloudTableClient.GetTableReference(UserDataTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki + "|" + username);
            string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, key);
            string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
            TableQuery<UserDataEntity> query = (new TableQuery<UserDataEntity>()).Where(combinedFilter);
            return table.ExecuteQuery<UserDataEntity>(query).FirstOrDefault();
		...
		private IList<UserDataEntity> GetUserDataEntities(string wiki, string username) {
            CloudTable table = _cloudTableClient.GetTableReference(UserDataTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki + "|" + username);
            TableQuery<UserDataEntity> query = (new TableQuery<UserDataEntity>()).Where(partitionKeyFilter);
            return table.ExecuteQuery<UserDataEntity>(query).ToList();
		...
		public ComponentInformation Information {
			get { return new ComponentInformation("Azure Table Storage Users Storage Provider", "Threeplicate Srl", "4.0.5.143", "http://www.sunhorizon.info", null); }
		}
    ...
    internal class UserGroupEntity : TableEntity
    {
		// PartitionKey = wiki
		// RowKey = groupName
    ...
    internal class UserDataEntity : TableEntity
    {
