...
namespace ScrewTurn.Wiki.Plugins.AzureStorage {
	...
	/// </summary>
	public class AzureFilesStorageProvider : IFilesStorageProviderV60 {

		private IHostV50 _host;
		private string _wiki;
		private CloudBlobClient _client;

		#region IFilesStorageProviderV50 Members
		...
		public string[] ListFiles(string directory) {
			try {
				...
				if(BuildNameForBlobStorage(directory) != "") {
					...
					var directoryRef = containerRef.GetDirectoryReference(BuildNameForBlobStorage(directory));
                    blobItems = directoryRef.ListBlobs(useFlatBlobListing: false);
				}
				else {
                    blobItems = containerRef.ListBlobs(useFlatBlobListing: false);
				}
				List<string> files = new List<string>();
				foreach(IListBlobItem blobItem in blobItems) {
					string blobName = blobItem.Uri.PathAndQuery;
		...
		public bool StoreFile(string fullName, System.IO.Stream sourceStream, bool overwrite) {
            ...
            {
                var errorCode = ex.RequestInformation.ExtendedErrorInformation.ErrorCode;
                if (errorCode == BlobErrorCodeStrings.BlobAlreadyExists) return false;
				throw ex;
			}
		}
		...
		public FileDetails GetFileDetails(string fullName) {
			...
			try {
				...
				return new FileDetails(blobRef.Properties.Length, blobRef.Properties.LastModified.Value.ToLocalTime().DateTime);
			}
            catch (StorageException ex)
            {
                var errorCode = ex.RequestInformation.ExtendedErrorInformation.ErrorCode;
		...
		public bool RenameFile(string oldFullName, string newFullName) {
			...
			try {
				...
				var oldBlobRef = containerRef.GetBlockBlobReference(oldBlobName);
				...
				var newBlobRef = containerRef.GetBlockBlobReference(BuildNameForBlobStorage(newFullName));

                newBlobRef.StartCopy(oldBlobRef);
				oldBlobRef.Delete();
            ...
            {
                var errorCode = ex.RequestInformation.ExtendedErrorInformation.ErrorCode;
                if (errorCode == BlobErrorCodeStrings.BlobNotFound) throw new ArgumentException("Old File does not exist", "oldFullName");
                if (errorCode == BlobErrorCodeStrings.BlobAlreadyExists) throw new ArgumentException("New File already exists", "newFullName");
				throw ex;
			}
		}
		...
		public bool CreateDirectory(string path, string name) {
			...
			try {
				...
				directoryRef.GetBlockBlobReference(".stw.dat").UploadText("");
            ...
            {
                var errorCode = ex.RequestInformation.ExtendedErrorInformation.ErrorCode;
                if (errorCode == BlobErrorCodeStrings.BlobAlreadyExists) throw new ArgumentException("Directory already exists", "name");
                if (errorCode == BlobErrorCodeStrings.BlobNotFound) throw new ArgumentException("Directory does not exist", "path");
				throw ex;
			}
		}
		...
		public bool DeleteDirectory(string fullPath) {
			...
			try {
				...
				foreach(string blob in blobs) {
					var blobRef = containerRef.GetBlockBlobReference(blob);
					blobRef.Delete();
				}
		...
		public bool RenameDirectory(string oldFullPath, string newFullPath) {
			...
			try {
				...
				foreach(string blob in blobs) {
					...
					var newBlobReference = containerRef.GetBlockBlobReference(newBlobName);
					var oldBlobReference = containerRef.GetBlockBlobReference(blob);
                    newBlobReference.StartCopy(oldBlobReference);
		...
		public FileDetails GetPageAttachmentDetails(string pageFullName, string attachmentName) {
            ...
            {
                if ((ex.RequestInformation != null) &&
                        (ex.RequestInformation.HttpStatusCode == (int)HttpStatusCode.NotFound)) return null;
				throw ex;
			}
		}
		...
		public bool RenamePageAttachment(string pageFullName, string oldName, string newName) {
			...
			try {
				...
				if(newBlobName != null) throw new ArgumentException("New File already exists", "newFullName");
				var newBlobRef = containerRef.GetBlockBlobReference(BuildNameForBlobStorage(pageFullName) + "/" + BuildNameForBlobStorage(newName));
		...
		public ComponentInformation Information {
			get { return new ComponentInformation("Azure Blob Storage Files Storage Provider", "Threeplicate Srl", "4.0.5.143", "http://www.sunhorizon.info", null); }
		}
	...
	internal class FileRetrievalStatEntity : TableEntity {
		// PartitionKey = wiki
		// RowKey = filename
