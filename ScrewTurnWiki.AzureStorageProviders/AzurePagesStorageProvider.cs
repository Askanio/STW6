...
namespace ScrewTurn.Wiki.Plugins.AzureStorage {
	...
	/// </summary>
	public class AzurePagesStorageProvider :IPagesStorageProviderV60 {
		...
		#region IPagesStorageProviderV50 Members
		...
		private List<NamespacesEntity> GetNamespacesEntities(string wiki) {
			if(_namespacesList == null) {
                CloudTable table = _cloudTableClient.GetTableReference(NamespacesTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                TableQuery<NamespacesEntity> query = (new TableQuery<NamespacesEntity>()).Where(partitionKeyFilter);
                _namespacesList = table.ExecuteQuery<NamespacesEntity>(query).ToList();
		...
		private NamespacesEntity GetNamespacesEntity(string wiki, string namespaceName) {
			...
			if(!_namespaces.ContainsKey(namespaceName)) {
                ...
                string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, namespaceName);
                string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
                TableQuery<NamespacesEntity> query = (new TableQuery<NamespacesEntity>()).Where(combinedFilter);
                var entity = table.ExecuteQuery<NamespacesEntity>(query).FirstOrDefault();
                //var query = (from e in _context.CreateQuery<NamespacesEntity>(NamespacesTable).AsTableServiceQuery()
		...
		public NamespaceInfo AddNamespace(string name) {
			...
			try {
				...
				var namespacesEntity = new NamespacesEntity() {
					PartitionKey = _wiki,
					RowKey = name,
					DefaultPageFullName = null
				};
                CloudTable table = _cloudTableClient.GetTableReference(NamespacesTable);
                TableOperation insertOperation = TableOperation.Insert(namespacesEntity);
		...
		public NamespaceInfo RenameNamespace(NamespaceInfo nspace, string newName) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(PagesContentsTable);
				...
				foreach(PagesContentsEntity entity in pagesInNamespace) {
					...
					foreach(PagesContentsEntity revision in allRevisions) {
                        ...
                        batchOperation.Delete(revision);
                        batchOperation.Insert(newPage);
						count = count + 2;
						if(count >= 98) {
                ...
                if (count > 0) table.ExecuteBatch(batchOperation);

                table = _cloudTableClient.GetTableReference(MessagesTable);
                batchOperation = new TableBatchOperation();
				foreach(PagesContentsEntity entity in pagesInNamespace) {
					string newPageFullName = NameTools.GetFullName(newName, NameTools.GetLocalName(entity.PageFullName));
					...
					foreach(MessageEntity message in messages) {
                        ...
                        batchOperation.Insert(newMessage);
						count++;
						if(count >= 99) {
                            table.ExecuteBatch(batchOperation);
                            batchOperation = new TableBatchOperation();
                ...
                table = _cloudTableClient.GetTableReference(CategoriesTable);
                batchOperation = new TableBatchOperation();
				...
				foreach(CategoriesEntity categoryEntity in categoryEntities) {
                    ...
                    batchOperation.Insert(newCategoryEntity);
                    batchOperation.Delete(categoryEntity);
				}
                if (batchOperation.Count > 0)
                    table.ExecuteBatch(batchOperation);

                table = _cloudTableClient.GetTableReference(NavigationPathsTable);
                batchOperation = new TableBatchOperation();
				...
				foreach(NavigationPathEntity navigationPathEntity in navigationPathEntities) {
					var newNavigationPathEntity = new NavigationPathEntity() {
                    ...
                    batchOperation.Insert(newNavigationPathEntity);
                    batchOperation.Delete(navigationPathEntity);
				}
                if (batchOperation.Count > 0)
                    table.ExecuteBatch(batchOperation);

				var newNamespaceEntity = new NamespacesEntity();
                ...
                table = _cloudTableClient.GetTableReference(NamespacesTable);
                batchOperation = new TableBatchOperation();
                batchOperation.Insert(newNamespaceEntity);
                batchOperation.Delete(oldNamespaceEntity);
                table.ExecuteBatch(batchOperation);
		...
		public bool RemoveNamespace(NamespaceInfo nspace) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(NamespacesTable);
                TableOperation operation = TableOperation.Delete(nspaceEntity);
                table.Execute(operation);
		...
		public PageContent MovePage(string pageFullName, NamespaceInfo destination, bool copyCategories) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(CategoriesTable);
                var batchOperation = new TableBatchOperation();
				...
				foreach(CategoryInfo category in pageCategories) {
                    ...
                    batchOperation.Replace(categoryEntity);
					if(copyCategories) {
						...
						if(newCategoryEntity == null) {
                            ...
                            batchOperation.Replace(newCategoryEntity);
						}
					}
				}
                if (batchOperation.Count > 0)
                ...
                table = _cloudTableClient.GetTableReference(PagesContentsTable);
                batchOperation = new TableBatchOperation();
				int count = 0;
				foreach(PagesContentsEntity entity in oldPagesContentsEntities) {
                    ...
                    batchOperation.Insert(newPageEntity);
					count = count + 2;
					if(count >= 98) {
                        table.ExecuteBatch(batchOperation);
                        batchOperation = new TableBatchOperation();
		...
		private CategoriesEntity GetCategoriesEntity(string wiki, string categoryFullName) {
            CloudTable table = _cloudTableClient.GetTableReference(CategoriesTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
            string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, categoryFullName);
            string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
            TableQuery<CategoriesEntity> query = (new TableQuery<CategoriesEntity>()).Where(combinedFilter);
            return table.ExecuteQuery<CategoriesEntity>(query).FirstOrDefault();
		...
		private List<CategoriesEntity> GetCategoriesEntities(string wiki) {
            CloudTable table = _cloudTableClient.GetTableReference(CategoriesTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
            TableQuery<CategoriesEntity> query = (new TableQuery<CategoriesEntity>()).Where(partitionKeyFilter);
            return table.ExecuteQuery<CategoriesEntity>(query).ToList();
            //var query = (from e in _context.CreateQuery<CategoriesEntity>(CategoriesTable).AsTableServiceQuery()
		...
		private List<CategoriesEntity> GetCategoriesEntities(string wiki, string namespaceName) {
            ...
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
            TableQuery<CategoriesEntity> query = (new TableQuery<CategoriesEntity>()).Where(partitionKeyFilter);
            var entities = table.ExecuteQuery<CategoriesEntity>(query).ToList();
            //var query = (from e in _context.CreateQuery<CategoriesEntity>(CategoriesTable).AsTableServiceQuery()
            //             where e.PartitionKey.Equals(_wiki)
            //             select e).AsTableServiceQuery();
		...
		public CategoryInfo AddCategory(string nspace, string name) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(CategoriesTable);
                TableOperation operation = TableOperation.Insert(categoriesEntity);
                table.Execute(operation);
		...
		public CategoryInfo RenameCategory(CategoryInfo category, string newName) {
			...
			try {
                ...
                var batchOperation = new TableBatchOperation();
                batchOperation.Insert(newCategoryEntity);
                batchOperation.Delete(oldCategoryEntity);
                table.ExecuteBatch(batchOperation);
		...
		public CategoryInfo MergeCategories(CategoryInfo source, CategoryInfo destination) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(CategoriesTable);
                var batchOperation = new TableBatchOperation();
                batchOperation.Replace(destinationCategoryEntity);
                batchOperation.Delete(sourceCategoryEntity);
                table.ExecuteBatch(batchOperation);
		...
		private PagesContentsEntity GetPagesContentsEntityByRevision(string wiki, string pageFullName, string revision) {
			...
			if(!_pagesContentCache[wiki].ContainsKey(pageFullName + "|" + revision)) {
                CloudTable table = _cloudTableClient.GetTableReference(PagesContentsTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, pageFullName + "|" + revision);
                string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
                TableQuery<PagesContentsEntity> query = (new TableQuery<PagesContentsEntity>()).Where(combinedFilter);
                var entity = table.ExecuteQuery<PagesContentsEntity>(query).FirstOrDefault();
				...
				if(entity == null) {
					...
					if(entity.BlobReference != null) {
						CloudBlockBlob blob = _containerRef.GetBlockBlobReference(entity.BlobReference);
		...
		private List<PagesContentsEntity> GetPagesContentEntities(string wiki, string pageFullName) {
			if(!allVersionRetrieved.Contains(wiki + "|" + pageFullName)) {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(PagesContentsTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                string pageFullNameFilter = TableQuery.GenerateFilterCondition("PageFullName", QueryComparisons.Equal, pageFullName);
                string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, pageFullNameFilter);
                TableQuery<PagesContentsEntity> query = (new TableQuery<PagesContentsEntity>()).Where(combinedFilter);
                var entities = table.ExecuteQuery<PagesContentsEntity>(query).ToList();
                //var query = (from e in _context.CreateQuery<PagesContentsEntity>(PagesContentsTable).AsTableServiceQuery()
		...
		private List<PagesContentsEntity> GetAllPagesContentsEntities(string wiki, string nspace) {
			if(!(allPagesContentRetrieved && _pagesContentCache != null && _pagesContentCache.ContainsKey(wiki))) {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(PagesContentsTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
                string revisionFilter = TableQuery.GenerateFilterCondition("Revision", QueryComparisons.Equal, CurrentRevision);
                string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, revisionFilter);
                TableQuery<PagesContentsEntity> query = (new TableQuery<PagesContentsEntity>()).Where(combinedFilter);
                var entities = table.ExecuteQuery<PagesContentsEntity>(query).ToList();
		...
		private void BuildPageContentEntity(PagesContentsEntity entity, string pageFullname, string revision, DateTime creationUniversalDateTime, string title,
			...
			if(Encoding.UTF8.GetByteCount(content) > 60 * 1024) {
				...
				CloudBlockBlob blob = _containerRef.GetBlockBlobReference(blobName);
		...
		private void DeleteOldBlobReference(string blobReference) {
			_containerRef.GetBlockBlobReference(blobReference).DeleteIfExists();
		}
		...
		public bool DeleteDraft(string fullName) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(PagesContentsTable);
                TableOperation operation = TableOperation.Delete(pageContentEntity);
                table.Execute(operation);
		...
		public bool SetBackupContent(PageContent content, int revision) {
			...
			try {
				...
				if(pageContentEntity != null) {
                    batchOperation.Replace(pageContentEntity);
                    ...
                    batchOperation.Insert(pageContentEntity);
				}
		...
		public PageContent RenamePage(string fullName, string newName) {
			...
			try {
				...
				foreach(NavigationPathEntity navigationPathEntity in navigationPathEntities) {
                    ...
                    batchOperation.Replace(navigationPathEntity);
				}
                if (batchOperation.Count > 0)
                    table.ExecuteBatch(batchOperation);
				...
				foreach(PagesContentsEntity entity in allOldRevisions) {
                    ...
                    batchOperation.Delete(entity);
                    batchOperation.Insert(newEntity);
				...
				switch(saveMode) {
					case SaveMode.Draft:
						...
						if(draftPageContentEntity == null) {
                            ...
                            batchOperation.Insert(draftPageContentEntity);
						}
						else {
                            batchOperation.Replace(draftPageContentEntity);
					...
					default:
						...
						if(currentPageContentEntity != null) {
							if(saveMode == SaveMode.Backup) {
                                ...
                                TableOperation operation = TableOperation.Insert(backupPageContentEntity);
                                table.Execute(operation);
							}
							else {
                            ...
                            batchOperation.Replace(currentPageContentEntity);
						}
						else {
							currentPageContentEntity = new PagesContentsEntity();
                            batchOperation.Insert(currentPageContentEntity);
						}
		...
		public bool DeleteBackups(string pageFullName, int revision) {
			...
			try {
                ...
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
                string pageFullNameFilter = TableQuery.GenerateFilterCondition("PageFullName", QueryComparisons.Equal, pageFullName);
                string revisionFilter = TableQuery.GenerateFilterCondition("Revision", QueryComparisons.NotEqual, CurrentRevision);
                string draftFilter = TableQuery.GenerateFilterCondition("Revision", QueryComparisons.NotEqual, Draft);
                //string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
                string combinedFilter = string.Format("({1}) {0} ({2}) {0} ({3}) {0} ({4})", TableOperators.And, partitionKeyFilter, pageFullNameFilter, revisionFilter, draftFilter);
                TableQuery<PagesContentsEntity> query = (new TableQuery<PagesContentsEntity>()).Where(combinedFilter);
                var pageContentEntities = table.ExecuteQuery<PagesContentsEntity>(query).ToList();
				...
				for(int i = 0; i <= idx; i++) {
                    ...
                    batchOperation.Delete(pageContentEntity);
					if(pageContentEntity.BlobReference != null) DeleteOldBlobReference(pageContentEntity.BlobReference);
				}
                if (batchOperation.Count > 0)
                    table.ExecuteBatch(batchOperation);
				...
				if(revision != -1) {
					for(int i = revision + 1; i < pageContentEntities.Count; i++) {
						var pageContentEntity = pageContentEntities.First(c => c.Revision == i.ToString());
						if(pageContentEntity.BlobReference != null) pageContentEntity.BigContent = _containerRef.GetBlockBlobReference(pageContentEntity.BlobReference).DownloadText();
                        ...
                        batchOperation.Insert(newPageContentEntity);
                        table.ExecuteBatch(batchOperation);
					}
				}
		...
		private MessageEntity GetMessageEntity(string wiki, string pageFullName, int messageId) {
            CloudTable table = _cloudTableClient.GetTableReference(MessagesTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki + "|" + pageFullName);
            string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, messageId.ToString(CultureInfo.InvariantCulture));
            string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
            TableQuery<MessageEntity> query = (new TableQuery<MessageEntity>()).Where(combinedFilter);
            return table.ExecuteQuery<MessageEntity>(query).FirstOrDefault();
		...
		private IList<MessageEntity> GetMessagesEntities(string wiki, string pageFullName) {
            CloudTable table = _cloudTableClient.GetTableReference(MessagesTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki + "|" + pageFullName);
            TableQuery<MessageEntity> query = (new TableQuery<MessageEntity>()).Where(partitionKeyFilter);
            return table.ExecuteQuery<MessageEntity>(query).ToList();
            //var messagesQuery = (from e in _context.CreateQuery<MessageEntity>(MessagesTable).AsTableServiceQuery()
		...
		public bool BulkStoreMessages(string pageFullName, Message[] messages) {
			...
			try {
				...
				{
				    var entity = BuildMessageEntity(_wiki, pageFullName, message.ID, message.Username, message.Subject,
				        message.DateTime, message.Body, -1);
					...
					foreach(Message reply in message.Replies) {
                        ...
                        batchOperation.Insert(replayEntity);
					}
				}
                if (batchOperation.Count > 0)
                    table.ExecuteBatch(batchOperation);
		...
		public int AddMessage(string pageFullName, string username, string subject, DateTime dateTime, string body, int parent) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(MessagesTable);
                TableOperation operation = TableOperation.Insert(messageEntity);
                table.Execute(operation);
		...
		public bool RemoveMessage(string pageFullName, int id, bool removeReplies) {
			...
			try {
                ...
                batchOperation.Delete(messageEntity);
		...
		public bool ModifyMessage(string pageFullName, int id, string username, string subject, DateTime dateTime, string body) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(MessagesTable);
                TableOperation operation = TableOperation.Replace(messageEntity);
                table.Execute(operation);
		...
		private NavigationPathEntity GetNavigationPathEntity(string wiki, string fullName) {
            CloudTable table = _cloudTableClient.GetTableReference(NavigationPathsTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
            string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, fullName);
            string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
            TableQuery<NavigationPathEntity> query = (new TableQuery<NavigationPathEntity>()).Where(combinedFilter);
            return table.ExecuteQuery<NavigationPathEntity>(query).FirstOrDefault();
		...
		private List<NavigationPathEntity> GetNavigationPathEntities(string wiki, string nspaceName) {
            CloudTable table = _cloudTableClient.GetTableReference(NavigationPathsTable); 
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
            TableQuery<NavigationPathEntity> query = (new TableQuery<NavigationPathEntity>()).Where(partitionKeyFilter);
            var entities = table.ExecuteQuery<NavigationPathEntity>(query).ToList();
		...
		public NavigationPath AddNavigationPath(string nspace, string name, string[] pages) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(NavigationPathsTable);
                TableOperation operation = TableOperation.Insert(navigationPathEntity);
                table.Execute(operation);
		...
		public NavigationPath ModifyNavigationPath(NavigationPath path, string[] pages) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(NavigationPathsTable);
                TableOperation operation = TableOperation.Replace(navigationPathEntity);
                table.Execute(operation);
		...
		public bool RemoveNavigationPath(NavigationPath path) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(NavigationPathsTable);
                TableOperation operation = TableOperation.Delete(navigationPathEntity);
                table.Execute(operation);
				return true;
			}
			catch(Exception ex) {
		...
		private SnippetEntity GetSnippetEntity(string wiki, string snippetName) {
            CloudTable table = _cloudTableClient.GetTableReference(SnippetsTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
            string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, snippetName);
            string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
            TableQuery<SnippetEntity> query = (new TableQuery<SnippetEntity>()).Where(combinedFilter);
            return table.ExecuteQuery<SnippetEntity>(query).FirstOrDefault();
		...
		public Snippet[] GetSnippets() {
			try {
                CloudTable table = _cloudTableClient.GetTableReference(SnippetsTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
                TableQuery<SnippetEntity> query = (new TableQuery<SnippetEntity>()).Where(partitionKeyFilter);
                var entities = table.ExecuteQuery<SnippetEntity>(query).ToList();
		...
		public Snippet AddSnippet(string name, string content) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(SnippetsTable);
                TableOperation operation = TableOperation.Insert(snippetEntity);
                table.Execute(operation);
		...
		public Snippet ModifySnippet(string name, string content) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(SnippetsTable);
                TableOperation operation = TableOperation.Replace(snippetEntity);
                table.Execute(operation);
		...
		public bool RemoveSnippet(string name) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(SnippetsTable);
                TableOperation operation = TableOperation.Delete(snippetEntity);
                table.Execute(operation);
		...
		private ContentTemplateEntity GetContentTemplateEntity(string wiki, string contentTemplateName) {
            CloudTable table = _cloudTableClient.GetTableReference(ContentTemplatesTable);
            string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, wiki);
            string rowKeyFilter = TableQuery.GenerateFilterCondition("RowKey", QueryComparisons.Equal, contentTemplateName);
            string combinedFilter = TableQuery.CombineFilters(partitionKeyFilter, TableOperators.And, rowKeyFilter);
            TableQuery<ContentTemplateEntity> query = (new TableQuery<ContentTemplateEntity>()).Where(combinedFilter);
            return table.ExecuteQuery<ContentTemplateEntity>(query).FirstOrDefault();
		...
		public ContentTemplate[] GetContentTemplates() {
			try {
                CloudTable table = _cloudTableClient.GetTableReference(ContentTemplatesTable);
                string partitionKeyFilter = TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, _wiki);
                TableQuery<ContentTemplateEntity> query = (new TableQuery<ContentTemplateEntity>()).Where(partitionKeyFilter);
                var entities = table.ExecuteQuery<ContentTemplateEntity>(query).ToList();
		...
		public ContentTemplate AddContentTemplate(string name, string content) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(ContentTemplatesTable);
                TableOperation operation = TableOperation.Insert(contentTemplateEntity);
                table.Execute(operation);
		...
		public ContentTemplate ModifyContentTemplate(string name, string content) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(ContentTemplatesTable);
                TableOperation operation = TableOperation.Replace(contentTemplateEntity);
                table.Execute(operation);
		...
		public bool RemoveContentTemplate(string name) {
			...
			try {
                ...
                CloudTable table = _cloudTableClient.GetTableReference(ContentTemplatesTable);
                TableOperation operation = TableOperation.Delete(contentTemplateEntity);
                table.Execute(operation);
		...
		#region IStorageProviderV50 Members
		...
		public void Init(IHostV50 host, string config, string wiki) {
			...
			_containerRef.CreateIfNotExists();
		...
		public ComponentInformation Information {
			get { return new ComponentInformation("Azure Table Storage Pages Storage Provider", "Threeplicate Srl", "4.0.5.143", "http://www.sunhorizon.info", null); }
		}
        ...
        {
            //TODO:  TableStorage.StorageAccount(config).CreateCloudBlobClient();
        }
	...
	internal class NamespacesEntity : TableEntity {
		// PartitionKey = wiki
    ...
    internal class CategoriesEntity : TableEntity
    {
		// PartitionKey = wiki
		// RowKey = categoryFullName
    ...
    internal class PagesContentsEntity : TableEntity
    {
		// PartitionKey = wiki
    ...
    internal class NavigationPathEntity : TableEntity
    {
		// PartitionKey = wiki
		// RowKey = fullName
    ...
    internal class SnippetEntity : TableEntity
    {
		// PartitionKey = wiki
		// RowKey = snippetName
    ...
    internal class ContentTemplateEntity : TableEntity
    {
		// PartitionKey = wiki
		// RowKey = snippetName
