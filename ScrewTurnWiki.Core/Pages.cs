...
namespace ScrewTurn.Wiki {
	...
	public static class Pages {
		...
		public static List<NamespaceInfo> GetNamespaces(string wiki) {
			...
			int count = 0;
			foreach(IPagesStorageProviderV60 provider in Collectors.CollectorsBox.PagesProviderCollector.GetAllProviders(wiki)) {
				count++;
		...
		public static NamespaceInfo FindNamespace(string wiki, string name) {
			...
			IPagesStorageProviderV60 defProv = Collectors.CollectorsBox.PagesProviderCollector.GetProvider(GlobalSettings.DefaultPagesProvider, wiki);
		...
		public static NamespaceInfo FindNamespace(string name, IPagesStorageProviderV60 provider) {
			if(string.IsNullOrEmpty(name)) return null;
		...
		/// <returns><c>true</c> if the namespace is created, <c>false</c> otherwise.</returns>
		public static bool CreateNamespace(string wiki, string name, IPagesStorageProviderV60 provider) {
		...
		/// <returns>The correct <see cref="T:PageInfo" /> object, if any, <c>null</c> otherwise.</returns>
		public static PageContent FindPage(string fullName, IPagesStorageProviderV60 provider) {
			if(string.IsNullOrEmpty(fullName)) return null;
		...
		public static void DeleteDraft(string pageFullName, IPagesStorageProviderV60 provider) {
		...
		public static PageContent SetPageWithEmptyContent(string nspace, string name, IPagesStorageProviderV60 provider, DateTime dateTime) {
		...
		/// <param name="saveMode">The save mode.</param>
		/// <returns>The newly created <see cref="PageContent"/> or <c>null</c> if something was wrong.</returns>
		public static PageContent SetPageContent(string wiki, string nspace, string name, IPagesStorageProviderV60 provider, string title, string username, DateTime dateTime, string comment, string content,
			string[] keywords, string description, SaveMode saveMode) {
		...
		/// <param name="provider">The Provider.</param>
		/// <returns><c>true</c> if the category is created, <c>false</c> otherwise.</returns>
		public static bool CreateCategory(string wiki, NamespaceInfo nspace, string name, IPagesStorageProviderV60 provider) {
			string namespaceName = nspace != null ? nspace.Name : null;
		...
		/// <param name="provider">The Provider.</param>
		/// <returns><c>true</c> if the category is created, <c>false</c> otherwise.</returns>
		public static bool CreateCategory(string wiki, string nspace, string name, IPagesStorageProviderV60 provider) {
