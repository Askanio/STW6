	...
	public static class Themes {
		...
		public static List<String> ListThemes(string wiki, string provider) {
			...
			if(provider == "standard") {
				...
				foreach(IThemesStorageProviderV60 prov in Collectors.CollectorsBox.ThemesProviderCollector.GetAllProviders(wiki)) {
		...
		public static bool DeleteTheme(string wiki, string themeName) {
			foreach(IThemesStorageProviderV60 themeDeleteProvider in Collectors.CollectorsBox.ThemesProviderCollector.GetAllProviders(wiki)) {
		...
		public static string GetThemePath(string wiki, string themeName) {
			...
			string provider = values[0];
			string theme = values[values.Length-1];
			if(provider == "standard") return "Content/Themes/" + GetRelativePath(Path.Combine(GlobalSettings.ThemesDirectory, theme), theme) + "/";
