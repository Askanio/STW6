...
namespace ScrewTurn.Wiki {
	...
	public static class ProviderLoader {
		...
		internal const string UsersProviderInterfaceName = "ScrewTurn.Wiki.PluginFramework.IUsersStorageProviderV50";
		internal const string PagesProviderInterfaceName = "ScrewTurn.Wiki.PluginFramework.IPagesStorageProviderV50";
		internal const string FilesProviderInterfaceName = "ScrewTurn.Wiki.PluginFramework.IFilesStorageProviderV50";
		internal const string ThemesProviderInterfaceName = "ScrewTurn.Wiki.PluginFramework.IThemesStorageProviderV50";
		internal const string FormatterProviderInterfaceName = "ScrewTurn.Wiki.PluginFramework.IFormatterProviderV50";
		...
		public static T CreateInstance<T>(Assembly asm, Type type) where T : class, IProviderV60 {
			T instance;
			try {
		...
		public static void Initialize<T>(T instance, string configuration, string wiki) where T : class, IProviderV60 {
			bool enabled = true;
			if(typeof(T) == typeof(IFormatterProviderV50)) {
		...
		public static string LoadProviderConfiguration(string typeName, Type interfaceType, string wiki) {
			if(interfaceType.FullName == typeof(IFormatterProviderV50).FullName) {
		...
		public static void LoadFormatterProvidersFromAssembly(string assembly, out Type[] formatters) {
			...
			for(int i = 0; i < types.Length; i++) {
				...
				foreach(Type iface in interfaces) {
					if(iface == typeof(IFormatterProviderV50)) {
						frs.Add(types[i]);
