...
namespace ScrewTurn.Wiki {
	...
	public class ProviderUpdater {
		...
		private IGlobalSettingsStorageProviderV60 globalSettingsProvider;
		private List<IFormatterProviderV50> plugins;
		...
		public int UpdateAll() {
			...
			foreach(IFormatterProviderV50 plugin in plugins) {
				...
				if(status == UpdateStatus.NewVersionFound && !string.IsNullOrEmpty(newDllUrl)) {
					...
					if(!visitedUrls.Contains(newDllUrl.ToLowerInvariant())) {
						...
						if(DownloadAndUpdateDll(plugin, newDllUrl, dllName)) {
							...
							updatedDlls++;
							foreach(Configuration.Wiki wiki in globalSettingsProvider.GetAllWikis()) {
								ProviderLoader.SetUp<IFormatterProviderV50>(plugin.GetType(), Settings.GetProvider(wiki.WikiName).GetPluginConfiguration(plugin.GetType().FullName));
							}
						}
					}
					else {
