...
namespace ScrewTurn.Wiki {
	...
	public static class Users {
		...
		public static List<UserInfo> GetUsers(string wiki) {
			...
			int count = 0;
			foreach(IUsersStorageProviderV60 provider in Collectors.CollectorsBox.UsersProviderCollector.GetAllProviders(wiki)) {
				count++;
		...
		public static UserInfo FindUser(string wiki, string username) {
			...
			IUsersStorageProviderV60 defaultProvider = Collectors.CollectorsBox.UsersProviderCollector.GetProvider(GlobalSettings.DefaultUsersProvider, wiki);
			UserInfo temp = defaultProvider.GetUser(username);
			if(temp != null) return temp;

			// The try other providers
			temp = null;
			IUsersStorageProviderV60[] providers = Collectors.CollectorsBox.UsersProviderCollector.GetAllProviders(wiki);
			foreach(IUsersStorageProviderV60 p in providers) {
				IUsersStorageProviderV60 extProv = p as IUsersStorageProviderV60;
		...
		public static bool AddUser(string wiki, string username, string displayName, string password, string email, bool active, IUsersStorageProviderV60 provider) {
		...
		private static List<string> ListDirectories(IFilesStorageProviderV60 provider) {
		...
		private static void ListDirectoriesRecursive(IFilesStorageProviderV60 provider, string current, List<string> output) {
		...
		private static void RemovePermissions(string wiki, UserInfo user) {
			...
			foreach(IPagesStorageProviderV60 prov in Collectors.CollectorsBox.PagesProviderCollector.GetAllProviders(wiki)) {
		...
		{
            var result = (List<UserGroup>)Cache.GetCachedItem(wiki, "UserGroups", () => GetAllUserGroups(wiki));
			return result;
		}
        ...
        /// <param name="wiki">The wiki.</param>
        /// <returns>All the user groups, sorted by name.</returns>
        private static List<UserGroup> GetAllUserGroups(string wiki) {
			List<UserGroup> result = new List<UserGroup>(50);
            
			int count = 0;
			foreach(IUsersStorageProviderV60 prov in Collectors.CollectorsBox.UsersProviderCollector.GetAllProviders(wiki)) {
				count++;
		...
		public static List<UserGroup> GetUserGroups(IUsersStorageProviderV60 provider) {
		...
		public static bool AddUserGroup(string wiki, string name, string description, IUsersStorageProviderV60 provider) {
			if(provider == null) provider = Collectors.CollectorsBox.UsersProviderCollector.GetProvider(GlobalSettings.DefaultUsersProvider, wiki);
		...
		public static string UserLink(string wiki, string username, bool newWindow) {
			...
			if(u != null) {
				return @"<a " +
					...
					@"href=""" + UrlTools.BuildUrl(wiki, "User?Username=", Tools.UrlEncode(u.Username)) + @""">" +
					GetDisplayName(u) + "</a>";
			}
		...
		public static UserInfo TryAutoLogin(string wiki, HttpContext context) {
			// Try default provider first
			IUsersStorageProviderV60 defaultProvider =
				Collectors.CollectorsBox.UsersProviderCollector.GetProvider(GlobalSettings.DefaultUsersProvider, wiki) as IUsersStorageProviderV60;
		...
		public static void NotifyLogout(string wiki, string username) {
			...
			UserInfo user = FindUser(wiki, username);
			if(user != null) {
				IUsersStorageProviderV60 prov = user.Provider as IUsersStorageProviderV60;
		...
		public static bool SetEmailNotification(string wiki, UserInfo user, NamespaceInfo nspace, bool pageChanges, bool discussionMessages) {
            ...
            string namespaceName = nspace != null ? nspace.Name : Settings.GetRootNamespaceName(wiki);
            string lowercaseNamespace = nspace != null ? nspace.Name.ToLowerInvariant() : Settings.GetRootNamespaceName(wiki).ToLowerInvariant();
			...
			foreach(string entry in pageChangesEntries) {
				if(entry.ToLowerInvariant() == lowercaseNamespace) {
                    ...
                    if (entry == Settings.GetRootNamespaceName(wiki)) pageChangesResult.Add(Settings.GetRootNamespaceName(wiki));
			...
			foreach(string entry in discussionMessagesEntries) {
				if(entry.ToLowerInvariant() == lowercaseNamespace) {
                    ...
                    if (entry == Settings.GetRootNamespaceName(wiki)) discussionMessagesResult.Add(Settings.GetRootNamespaceName(wiki));
	    ...
	    /// <param name="pageChanges">A value indicating whether page changes should be notified.</param>
	    /// <param name="discussionMessages">A value indicating whether discussion messages should be notified.</param>
	    public static void GetEmailNotification(string wiki, UserInfo user, NamespaceInfo nspace, out bool pageChanges, out bool discussionMessages) {
			pageChanges = false;
			discussionMessages = false;

			if(user == null) return;

            string lowercaseNamespaces = nspace != null ? nspace.Name.ToLowerInvariant() : Settings.GetRootNamespaceName(wiki);
