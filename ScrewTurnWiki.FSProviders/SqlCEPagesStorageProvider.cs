...
namespace ScrewTurn.Wiki.Plugins.FSProviders {
	...
	/// </summary>
	public class SqlCEPagesStorageProvider : SqlPagesStorageProviderBase, IPagesStorageProviderV60 {

        private readonly ComponentInformation info = new ComponentInformation("SQL CE Pages Storage Provider", "Threeplicate Srl", "5.0.0.1", "http://www.sunhorizon.info", null);
		...
		public new void SetUp(IHostV50 host, string config) {
        ...
        /// <summary>
        /// Detects whether the table Namespace needs to be insert namespace.
        /// </summary>
        /// <returns><c>true</c> if an insert is needed, <c>false</c> otherwise.</returns>
        private bool NamespaceNeedsUpdate(string wikiName)
        {
            DbCommand cmd = GetCommand(_connString);
            cmd.CommandText = "select count(*) from [Namespace] where [Wiki] = @wiki";
            cmd.Parameters.Add(new SqlCeParameter("Wiki", wikiName));
		...
		protected override void CreateOrUpdateDatabaseIfNecessary() {
			...
			foreach(Configuration.Wiki wiki in host.GetGlobalSettingsStorageProvider().GetAllWikis()) {
                if (NamespaceNeedsUpdate(wiki.WikiName))
        ...
        public override void ValidateConfig(string config)
        {
            // TODO:
        }
    }
